...
- Added progressive lineage visibility with inline badges: lineage modal now shows only root + direct parents + sources by default, with hidden upstream models indicated by small badges on nodes (e.g., "⋯ 16"). Clicking badges expands one hop upstream. Source overlay edges (subtle green) automatically connect visible nodes to their upstream sources through hidden paths, ensuring source traceability without cluttering the graph. This provides a clean default view while maintaining full lineage visibility on demand.

### Fixed
- Fixed lineage edges disappearing in complex lineage graphs: edges were being hidden when their source or target nodes were marked as hidden by progressive expansion rules. The lineage modal now uses progressive visibility without removing nodes/edges from the graph, preventing edges from disappearing during expansion or navigation.
