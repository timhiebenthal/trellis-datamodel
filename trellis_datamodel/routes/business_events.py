...
async def add_event_annotation(
    event_id: str, request: AddAnnotationRequest = Body(...)
):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error adding annotation: {str(e)}"
        )


@router.delete(
    "/business-events/{event_id}/annotations/{index}", response_model=BusinessEvent
)
async def remove_event_annotation(event_id: str, index: int):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error removing annotation: {str(e)}"
        )
